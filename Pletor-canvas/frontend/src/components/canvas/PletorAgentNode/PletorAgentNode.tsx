...
function PletorAgentNode({ id, data }: NodeProps) {
  ...
  return (
    <div className="pletor-node pletor-node-agent" onClick={() => selectNode(id)}>
      <Handle type="target" position={Position.Left} style={{ background: 'oklch(62% 0.22 350)' }} />
      ...
      <Handle type="source" position={Position.Right} style={{ background: 'oklch(62% 0.22 350)' }} />
    </div>
  )
}
