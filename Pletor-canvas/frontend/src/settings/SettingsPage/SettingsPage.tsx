...
import SettingsFieldRow from '../../ui/SettingsFieldRow/SettingsFieldRow'
import { NODE_COLORS } from '../../canvas/canvas.types'
import type { PletorNodeType } from '../../canvas/canvas.types'
...
// Přehled canvasu — reálná data z canvasStore
function CanvasDashboard() {
  const nodes = useCanvasStore((s) => s.nodes)
  const edges = useCanvasStore((s) => s.edges)

  // Počty uzlů podle typu
...
function SettingsPage({ activeMenu }: SettingsPageProps) {
  ...
  // Sekce "project" zobrazí dashboard + nastavení
  ...
  function handleSave() {
    // Zustand persist ukládá automaticky — toto je vizuální potvrzení
        ...
        <div className="settings-fields">
          {section.fields.map((field) => (
            <SettingsFieldRow
              key={field.id}
              field={field}
              onChange={(value: string) => updateField(sectionId, field.id, value)}
            />
          ))}
        </div>
