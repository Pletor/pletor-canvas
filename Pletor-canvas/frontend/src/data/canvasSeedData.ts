import type { PletorNode, PletorEdge } from '../types/canvas.types'
...
export const SEED_NODES: PletorNode[] = [
  ...
  // API
  {
    ...
    data: { label: 'canvasApi.ts', nodeType: 'api', description: 'REST API klient (Fáze 1)', status: 'draft' },
  },
  ...
  // Agent
  {
    ...
    data: { label: 'Code Generator', nodeType: 'agent', description: 'Generuje kód z WorkFlowy instrukcí', status: 'draft' },
  },
  {
    id: 'agent-reviewer',
    type: 'agent',
  ...
  {
    id: 'integration-workflowy',
    type: 'integration',
    position: { x: 800, y: 540 },
    data: { label: 'WorkFlowy', nodeType: 'integration', description: 'Hierarchický mozek projektu', status: 'draft' },
  },
]
...
export const SEED_EDGES: PletorEdge[] = [
  ...
  // Store → API (API call, budoucí)
  { id: 'e-store-api', source: 'service-store', target: 'api-canvas', type: 'apiCall', data: { edgeType: 'apiCall' } },

  // Agent → WorkFlowy (data flow)
  { id: 'e-agent-wf', source: 'agent-codegen', target: 'integration-workflowy', type: 'dataFlow', animated: true, data: { edgeType: 'dataFlow' } },
